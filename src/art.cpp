...
static void add_child48(art_node48 *n, art_node **ref, unsigned char c, void *child)
{
	...
	{
		art_node256 *_new = (art_node256*)alloc_node(NODE256);
		for (int i = 0; i < 256; i++)
		{
			if (n->keys[i])
			{
				_new->children[i] = n->children[n->keys[i] - 1];
			}
		}
		copy_header((art_node*)_new, (art_node*)n);
		*ref = (art_node*)_new;
		free(n);
		add_child256(_new, ref, c, child);
	}
}
...
static void add_child16(art_node16 *n, art_node **ref, unsigned char c, void *child)
{
	...
	{
		art_node48 *_new = (art_node48*)alloc_node(NODE48);

		// Copy the child pointers and populate the key map
		memcpy(_new->children, n->children,
		       sizeof(void*)*n->n.num_children);
		for (int i = 0; i < n->n.num_children; i++)
		{
			_new->keys[n->keys[i]] = i + 1;
		}
		copy_header((art_node*)_new, (art_node*)n);
		*ref = (art_node*)_new;
		free(n);
		add_child48(_new, ref, c, child);
	}
}
...
static void add_child4(art_node4 *n, art_node **ref, unsigned char c, void *child)
{
	...
	{
		art_node16 *_new = (art_node16*)alloc_node(NODE16);

		// Copy the child pointers and the key map
		memcpy(_new->children, n->children,
		       sizeof(void*)*n->n.num_children);
		memcpy(_new->keys, n->keys,
		       sizeof(unsigned char)*n->n.num_children);
		copy_header((art_node*)_new, (art_node*)n);
		*ref = (art_node*)_new;
		free(n);
		add_child16(_new, ref, c, child);
	}
}
...
static void* recursive_insert(art_node *n, art_node **ref, const unsigned char *key, int key_len, void *value, int depth, int *old)
{
	...
	if (IS_LEAF(n))
	{
		...
		// _new value, we must split the leaf into a node4
		art_node4 *_new = (art_node4*)alloc_node(NODE4);
		...
		_new->n.partial_len = longest_prefix;
		memcpy(_new->n.partial, key + depth, min(MAX_PREFIX_LEN, longest_prefix));
		// Add the leafs to the _new node4
		*ref = (art_node*)_new;
		add_child4(_new, ref, l->key[depth + longest_prefix], SET_LEAF(l));
		add_child4(_new, ref, l2->key[depth + longest_prefix], SET_LEAF(l2));
		return NULL;
	}
	...
	if (n->partial_len)
	{
		...
		art_node4 *_new = (art_node4*)alloc_node(NODE4);
		*ref = (art_node*)_new;
		_new->n.partial_len = prefix_diff;
		memcpy(_new->n.partial, n->partial, min(MAX_PREFIX_LEN, prefix_diff));

		// Adjust the prefix of the old node
		if (n->partial_len <= MAX_PREFIX_LEN)
		{
			add_child4(_new, ref, n->partial[prefix_diff], n);
		...
		else
		{
			n->partial_len -= (prefix_diff + 1);
			art_leaf *l = minimum(n);
			add_child4(_new, ref, l->key[depth + prefix_diff], n);
			memcpy(n->partial, l->key + depth + prefix_diff + 1,
			       min(MAX_PREFIX_LEN, n->partial_len));
		}

		// Insert the _new leaf
		art_leaf *l = make_leaf(key, key_len, value);
		add_child4(_new, ref, key[depth + prefix_diff], SET_LEAF(l));
		return NULL;
	}
...
/**
 * Inserts a _new value into the ART tree
 * @arg t The tree
 * @arg key The key
 * @arg key_len The length of the key
 * @arg value Opaque value.
 * @return NULL if the item was _newly inserted, otherwise
	...
	if (n->n.num_children == 37)
	{
		...
		*ref = (art_node*)_new;
		...
		for (int i = 0; i < 256; i++)
		{
			if (n->children[i])
			{
				_new->children[pos] = n->children[i];
				_new->keys[i] = pos + 1;
				pos++;
			}
		}
	...
	if (n->n.num_children == 12)
	{
		...
		*ref = (art_node*)_new;
		...
		for (int i = 0; i < 256; i++)
		{
			...
			{
				_new->keys[child] = i;
				_new->children[child] = n->children[pos - 1];
				child++;
			}
		}
		free(n);
...
static void remove_child16(art_node16 *n, art_node **ref, art_node **l)
{
	...
	if (n->n.num_children == 3)
	{
		...
		*ref = (art_node*)_new;
		copy_header((art_node*)_new, (art_node*)n);
		memcpy(_new->keys, n->keys, 4);
		memcpy(_new->children, n->children, 4 * sizeof(void*));
		free(n);
	}
}
